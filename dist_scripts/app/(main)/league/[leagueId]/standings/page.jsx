...
export default function StandingsPage(_a) {
    var params = _a.params;
    var leagueId = params.leagueId;
    var _b = useState(true), isLoading = _b[0], setIsLoading = _b[1];
    var _c = useState([]), standings = _c[0], setStandings = _c[1];
        ...
        <div className="card-body">
          <div className="overflow-x-auto">
            <table className="table table-zebra">
              ...
              <tbody>
                {standings.map(function (team, index) { return (<tr key={team.teamId} className={team.teamId === 'team1' ? 'bg-primary/10' : ''}>
                    <td>{index + 1}</td>
                    <td>
                      <div className="font-medium">{team.name}</div>
                      <div className="text-sm text-base-content/70">{team.owner}</div>
                    </td>
                    <td>{team.wins}-{team.losses}{team.ties > 0 ? "-".concat(team.ties) : ''}</td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-base-300 rounded-lg p-4">
              ...
              <ol className="list-decimal list-inside space-y-1">
                {standings.slice(0, 4).map(function (team) { return (<li key={team.teamId} className={team.teamId === 'team1' ? 'font-medium text-primary' : ''}>
            ...
            <div className="border border-base-300 rounded-lg p-4">
              ...
              <ol className="list-decimal list-inside space-y-1" start={5}>
                {standings.slice(4).map(function (team) { return (<li key={team.teamId} className={team.teamId === 'team1' ? 'font-medium text-primary' : ''}>
                    {team.name} ({team.wins}-{team.losses})
                  </li>); })}
              </ol>
            </div>
