...
jest.mock('next/server', () => {
  class MockNextRequest {
    ...
    nextUrl: URL;
    method: string;
    body?: unknown; // Store raw body for .json()
    ...
    constructor(input: string | URL | RequestInfo, init?: RequestInit) {
      ...
      this.body = init?.body;
      this.ip = (init as RequestInit & { ip?: string })?.ip || '127.0.0.1'; // Allow mocking IP
    }
...
// Mock DAL functions
const mockedFindUserByEmail: jest.Mock = jest.fn(); // Declare without initializing
jest.mock('@/lib/dal/user.dal', () => ({
  findUserByEmail: (...args: unknown[]) => mockedFindUserByEmail(...args), // Use a wrapper
...
describe('POST /api/auth/login', () => {
  ...
  beforeEach(() => {
    ...
    (mockRequest as NextRequest & { json: jest.Mock; ip: string }).json = jest.fn().mockResolvedValue({ email: 'test@example.com', password: 'Password123!' });
    (mockRequest as NextRequest & { ip: string }).ip = '127.0.0.1'; // for rate limiter
  });
  ...
  it('should return 405 if method is not POST', async () => {
    ...
    (getRequest as NextRequest & { ip: string }).ip = '127.0.0.1'; // Set IP for rate limiter if applicable
  ...
  it('should return 400 for invalid JSON body', async () => {
    ...
    (mockRequest as NextRequest & { json: jest.Mock }).json = jest.fn().mockRejectedValueOnce(new Error('Invalid JSON'));
  ...
  it('should return 400 for invalid DTO (missing email)', async () => {
    (mockRequest as NextRequest & { json: jest.Mock }).json = jest.fn().mockResolvedValueOnce({ password: 'Password123!' });
    const response = await loginRouteHandler(mockRequest);
    expect(response.status).toBe(400);
    const body = await response.json();
